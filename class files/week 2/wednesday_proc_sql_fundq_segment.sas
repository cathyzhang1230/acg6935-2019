...
/* setting label and formatting on new variables */
proc sql;
	create table myTable as 
		select sich, fyear, median(sale) as sale_median LABEL="Industry median sales" format=comma20. 
		from a_funda group by sich, fyear;
quit;

/* compare with this -- appends */
proc sql;
	create table myTable2 as 
		select *, median(sale) as sale_median LABEL="Industry median sales" format=comma20. 
		from a_funda group by sich, fyear;
quit;


/* using 'calculated' to refer to a newly constructed variable */
proc sql;
	create table myTable3 as 
		select *, median(sale) as sale_median LABEL="Industry median sales" format=comma20. ,
		max(sale) as sale_max, min(sale) as sale_min,
		/* create a dummy, set to 1 if firm size exceeds median size for industry-year */
		(sale > calculated sale_median) as large
		from a_funda group by sich, fyear;
quit;

/* 'where' filters the input set, 'having' can filter the output dataset 
	(having is usually combined with a group by -- otherwise the filtering is on the full dataset
...
proc sql;
	create table myTable4 as 
		...
		group by sich, fyear
		having
			/* only select observations with the highest sale in the industry-year */
		 	sale = max(sale)
			/* only select industries with more than 5 obs */
			and count(*) > 5;				
quit;

/* as above,but using sale_max */
proc sql;
	create table myTable4 as 
		select *, median(sale) as sale_median LABEL="Industry median sales" format=comma20. ,
		max(sale) as sale_max, min(sale) as sale_min
...
/* add lagged data to a_funda  (add ni from 1 year before) */

proc sql;
 	create table e_self as
	select a.*, b.ni as ni_prev, b.fyear as fyear_prev /* for debugging */
	/* note that b_fundq is both a and b: a self join */
	from a_funda a left join a_funda b
    on  a.gvkey = b.gvkey /* same firm */
	and a.fyear -1 = b.fyear ;/* b.roa needs to be 1 year before a.fyearq */	
quit;
